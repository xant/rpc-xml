...
#
# adapted to mod_perl2 by xant@cpan.org
#
...
#   $Id:$
#
#   Description:    This package implements a RPC server as an Apache/mod_perl2
...
BEGIN
{
    $Apache2::RPC::Server::INSTALL_DIR = (File::Spec->splitpath(__FILE__))[1];
    %Apache2::RPC::Server::SERVER_TABLE = ();
}

$Apache2::RPC::Server::VERSION = '1.30';

sub version { $Apache2::RPC::Server::VERSION }

sub INSTALL_DIR { $Apache2::RPC::Server::INSTALL_DIR }

# Return a list (not list reference) of currently-known server objects,
# represented as the text-keys from the hash table.
sub list_servers { keys %Apache2::RPC::Server::SERVER_TABLE }
...
#                                                 invoked in
#                   $r        in      ref       Blessed Request object
#
#   Globals:        $DEF_OBJ
#
...
sub handler ($$)
{
    ...
    elsif ($r->method eq 'POST')
    {
        ...
        return DECLINED unless ($r->headers_in->{'Content-Type'} =~ m/text\/xml/i);
        $compress = $srv->compress;
        $do_compress = 1
            if ($compress and
                ($r->headers_in->{'Content-Encoding'} || '') =~ $srv->compress_re);
        ...
        $length = $r->headers_in->{'Content-Length'};
        ...
        else
        {
            ...
            $hdrs_out->{'X-Content-Length'} = length($content);
...
sub init_handler ($$)
{
    ...
    $_->child_started(1) for (values %Apache2::RPC::Server::SERVER_TABLE);
...
sub new
{
    ...
    %argz = @argz;
    $R      = $argz{apache} || Apache2::RequestUtil->request; delete $argz{apache};
    ...
    if ($argz{xpl_path})
    {
        push(@{$argz{xpl_path}}, $Apache2::RPC::Server::INSTALL_DIR);
    }
    else
    {
        $argz{xpl_path} = [ $Apache2::RPC::Server::INSTALL_DIR ];
    }
    ...
    $Apache2::RPC::Server::SERVER_TABLE{$servid} = $self;
    $self;
}
...
sub get_server
{
    ...
    else
    {
        ...
        # the fly.
        return $Apache2::RPC::Server::SERVER_TABLE{$r} ||
...
Apache2::RPC::Server - A subclass of RPC::XML::Server tuned for mod_perl2

=head1 SYNOPSIS

    # In httpd.conf:
    PerlModule Apache2::RPC::Server
    PerlSetVar RpcMethodDir /var/www/rpc:/usr/lib/perl5/RPC-shared
    PerlChildInitHandler Apache2::RPC::Server->init_handler
    ...
    <Location /RPC>
        SetHandler perl-script
        PerlHandler Apache2::RPC::Server
    </Location>
    ...
    use Apache2::RPC::Server;

=head1 DESCRIPTION

The B<Apache2::RPC::Server> module is a subclassing of B<RPC::XML::Server> that
is tuned and designed for use within Apache with mod_perl2.
...
following methods are either added or overloaded by B<Apache2::RPC::Server>:
...
packages to easily subclass B<Apache2::RPC::Server>.
...
Methods are provided to an B<Apache2::RPC::Server> object in three ways:
...
package are loaded into the table. The B<Apache2::RPC::Server> objects get a
...
environment:
    ...
    # First, create and configure some Apache2::RPC::Server objects

    # One regular one, with the standard settings:
    $main::defobj = Apache2::RPC::Server->new(path         => '/RPC',
                                             auto_methods => 1,
                                             auto_updates => 1);
    # One version without the default methods, and no auto-actions
    $main::secobj = Apache2::RPC::Server->new(no_default => 1,
                                             path => '/rpc-secured');
...
for each of the locations to be handed the B<Apache2::RPC::Server> object
